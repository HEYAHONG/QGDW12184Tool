
#include "qgdw12184_crc.h"

// -----------------------------------------------------------------------------
// DESCRIPTION: RTU CRC 校验的高位字节表
// -----------------------------------------------------------------------------
static const unsigned char auchCRCHi[] =
{
    
} ;
// -----------------------------------------------------------------------------
// DESCRIPTION: RTU CRC 校验的低位字节表
// -----------------------------------------------------------------------------
static const unsigned char auchCRCLo[] =
{
    
};

//校验函数：
static uint16_t RTU_CRC( unsigned char * puchMsg, size_t usDataLen )
{
    unsigned char uchCRCHi; // high 字节 of CRC initialized
    unsigned char uchCRCLo; // low 字节 of CRC initialized
    unsigned uIndex; // will index into CRC lookup table
    uchCRCHi = 0xFF;
    uchCRCLo = 0xFF;
    while ( usDataLen-- )
    {
        // calculate the CRC
        uIndex = uchCRCLo ^ (unsigned char)( *puchMsg++ );
        uchCRCLo = uchCRCHi ^ auchCRCHi[uIndex];
        uchCRCHi = auchCRCLo[uIndex];
    }
    return ( uchCRCHi << 8 | uchCRCLo );
}

uint16_t qgdw12184_crc_claculate(uint8_t *buff,size_t buff_len)
{
    if(buff==NULL || buff_len == 0)
    {
        return 0;
    }
    return RTU_CRC(buff,buff_len);
}

bool qgdw12184_crc_check(uint8_t *frame,size_t frame_len)
{
    if(frame==NULL || frame_len <= 2)
    {
        return false;
    }

    uint16_t frame_crc=frame[frame_len-2];
    frame_crc<<=8;
    frame_crc+=frame[frame_len-1];
    return frame_crc == qgdw12184_crc_claculate(frame,frame_len-2);
}

void qgdw12184_crc_append(uint8_t *frame,size_t frame_len)
{
    if(frame==NULL || frame_len <= 2)
    {
        return;
    }
    uint16_t frame_crc=qgdw12184_crc_claculate(frame,frame_len-2);
    frame[frame_len-2]=(0xFF & (frame_crc>>8));
    frame[frame_len-1]=(0xFF & (frame_crc));
}
